...
export const load: PageServerLoad = async ({ fetch, params, url }) => {
  ...
  const [summary, topics, topPosts, chamber, trend] = await Promise.all([
    api(fetch, `/states/${params.state}`, filters),
    api(fetch, `/states/${params.state}/topics`, filters),
    api(fetch, `/states/${params.state}/top-posts`, { limit: 10, ...filters }),
    api(fetch, '/chamber', { state: params.state.toUpperCase(), party: context.party, topic: context.topic }),
    api(fetch, `/states/${params.state}/trend`, filters)
  ]);
