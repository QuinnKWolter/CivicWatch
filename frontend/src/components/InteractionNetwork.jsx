...
function InteractionNetwork({ startDate, endDate, selectedTopics, selectedMetric, legislator, keyword }) {
  ...
  useEffect(() => {
    ...
    // 1) Filter links by topics and date range
    ...
    if (typeof keyword === "string" && keyword.trim() !== "") {
      ...
      links = links.filter(l => {
        // Check post text (handles both single 'text' and array 'texts' properties)
        if (l.text?.toLowerCase().includes(lower)) return true;
        if (Array.isArray(l.texts) && l.texts.some(t => t.toLowerCase().includes(lower))) return true;

        // Check topics
        if (l.topics?.some(t => t.toLowerCase().includes(lower))) return true;

        // Check legislator name, state, or region
        const src = data.nodes.find(n => n.legislator_id === l.source_legislator_id);
        const tgt = data.nodes.find(n => n.legislator_id === l.target_legislator_id);
    ...
    // 3) Filter links by region if selected
    ...
    // 4) Prune nodes and links based on filters
    const inIds = new Set();
    links.forEach(l => {
    ...
    // Filter for a specific legislator if selected
    ...
    // Re-index nodes and filter links to ensure both source and target exist
    ...
    // Final pruning of nodes to ensure they are still in active links
    ...
    const finalNodeIndex = Object.fromEntries(nodes.map((n, i) => [n.legislator_id, i]));

    // 5) Build the final matrix for the chord diagram
    const N = nodes.length;
    const matrix = Array.from({ length: N }, () => Array(N).fill(0));
    links.forEach(l => {
        const i = finalNodeIndex[l.source_legislator_id];
        const j = finalNodeIndex[l.target_legislator_id];
        const v = filters.interactionType === 'all' ? l.value : (l[filters.interactionType] || 0);
